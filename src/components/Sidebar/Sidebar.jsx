...
const Sidebar = () => {
    ...
    return (
        <div className='inline-flex flex-col justify-between min-h-screen px-4 py-2 bg-xBg'>
            <div id="top" className="pt-5">
                ...
                <div id="new-chat" className='inline-flex items-center cursor-pointer gap-4 px-2 py-2 mt-10 bg-[#e6eaf1] rounded-3xl'>
                    <MdAdd size={25} color='#444' />
                    {extended ? <p className='text-sm pr-2 font-medium text-zinc-700'>New Chat</p> : null}
                </div>
                {
                    extended ? <div id="recent" className='flex flex-col'>
                        <p id="recent-title" className='mb-5 font-bold mt-7 text-zinc-700'>Recent</p>
                        <div id="recent-entry" className='flex items-center gap-4 p-2 rounded-md cursor-pointer hover:bg-zinc-300 w-fit '>
                            <IoChatboxOutline size={20} className='text-zinc-600' />
                            <p className='font-medium text-zinc-600'>What is react...</p>
                            <BsThreeDotsVertical size={25} className='p-1 ml-10 rounded-full hover:bg-zinc-400 hover:text-zinc-800 text-xBg ' />
                        </div>
                    </div> : null
                }
